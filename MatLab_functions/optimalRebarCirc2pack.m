function [Mr_col,Inertia,bestArea,bestCost,bestdiagram,bestnv,bestobar,...
    bestEf,bestType,bestDisposition,bestc,criticalEf]=optimalRebarCirc2pack...
...
%  bestEf,bestType,bestDisposition,bestc]=optimalRebarCirc2pack...
%  (diam,rec,act,Es,npdiag,fdpc,pu_col_circ,load_conditions,...
%  optionsBar,wac,height,plotRebarDesign)
%
%-------------------------------------------------------------------------
...
% concrete column, with rebars in packages of two.
% 
...
%
%         criticalEf:           corresponding resistance efficiency for the
%                               critical bending-compression load:
%
%                               [Pu,Mu,Pr,Mr,Ef]
%
...
%
%------------------------------------------------------------------------
% LAST MODIFIED: L.F.Veduzco    2023-12-19
...
for i=1:ntrebar
    ...
    if fc<2000 % units: (Kg,cm)
        ...
        dv3=3/8*2.54; % stirrups diameter
    else       % units: (lb,in)
        sepMin=max([1.5, 3/2*dv]);
        dv3=3/8; % stirrups diameter
    end
    % There is a limit of the number of rebars that can be laid out
    % on the column's inner boundary
    
    maxVarillas=fix((diam-2*rec-2*dv3-2*dv)/(sepMin+dv))+1;
    nvla=nv*0.5;
    if (maxVarillas)<nvla % the total number of required rebars
                            % is placed in two lanes
        continue;
    else
        noptions=noptions+1;
        ...
        [dispositionRebar]=RebarDispositionCirc2pack(diam,rec,dv,nv);
        ...
        if ast<=bestArea && maxef<bestEf
            ...
            criticalEf=effTable(bestcritical,:);
            bestEf=maxef;
