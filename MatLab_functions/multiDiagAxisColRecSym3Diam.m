function [tablaEff,iloadmax,maxLoadCondition,maxgamma,diagramIntAxis1,...
    rotdispositionRebar,rotSection,cmax]=multiDiagAxisColRecSym3Diam(b,h,...
    loadConditions,db3,npdiag,fy,fcu,Es,nb1,nb2,nb3,distrRebar)
%------------------------------------------------------------------------
% Syntax:
...
%
% OUTPUT: tablaEff:             is the summary table of the structural
...
iloadmax=1;
maxef=-inf;
nloads=length(loadConditions(:,1));
for i=1:nloads
    %% Interaction diagram in the load direction
    Pu=loadConditions(i,2);
    Mux=loadConditions(i,3);
    Muy=loadConditions(i,4);

    [diagramIntAxis,pot,poc,NeutrAx,newditrRebar,...
    newCoordCorners,gamma]=InteracDiagAxColSym3Diam...
    (npdiag,db3,b,h,fy,fcu,Es,nb1,nb2,nb3,distrRebar,Mux,Muy);

    newCoordCorners(5,:)=newCoordCorners(1,:);    % to close the drawing
                                                  % of the rotated section
    %% Structural efficiency
    Mxy=sqrt(Mux^2+Muy^2);
    transLoadCondition=[1 Pu Mxy];
    [ef,tablaEffLoad,c]=effColsRot1DirecLS(diagramIntAxis,...
                                transLoadCondition,NeutrAx);
    ...
    if ef>maxef
        ...
        rotdispositionRebar=newditrRebar;
        rotSection=newCoordCorners;
        cmax=c;
        maxLoadCondition=transLoadCondition;
    end
end