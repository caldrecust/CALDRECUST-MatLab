function [bestPerformance,bestLenRebar,bestsepRebar,bestPosition,bestdbc9,...
    ...
    bestCFA]=GNNPSOBeamsRebarLM1DLayer(b,h,span,brec,hrec,hagg,pmin,pmax,rebarAvailable,...
    fcu,load_conditions,fy,wac,cutLoc,dbcc,nbcc,dblow,nblow,Ao3,Wfac,graphConvPlot,...
    ispan,nMaxIter,numberOfParticles)
...
for j=1:nMaxIter
    ...
    for i=1:numberOfParticles
        ...
        if all([dbc(1)>=dbc(2),dbc(2)>=dbc(3),...
            ...
            [nb1m,nb2m,nb3m,isfeasible]=nbSimple3D3L(nbmaxm,Ao3(2),abc(4:6),nbcc);
...
if graphConvPlot==1 && isempty(cbestLeft)==0
    ...
    validPeformItems=[];
    for i=1:nMaxIter
        if performanceHist(i)<1e10
            performanceHistF=[performanceHistF,performanceHist(i)*wac];
            validPeformItems=[validPeformItems,i];
        end
    end
    figure(4)
    if sum(nbcc)==0
        plot(validPeformItems,...
           performanceHistF.*wac,'-o')
        hold on
        legend('Span 1')
    else
        plot(validPeformItems,...
           performanceHistF.*wac,'-o','DisplayName',strcat('Span',num2str(ispan)))
        hold on
    end
    xlabel('Iteration')
