function [volRebarSpans,LenRebarL,LenRebarM,LenRebarR,sepRebarSpans,db9Spans,...
  ...
  SASOOptimMSFSBeamsRebarSimple(b,h,span,brec,hrec,hagg,pmin,pmax,rebarAvailable,...
  fcu,load_conditions,fy,wac,cutxLoc,Ao3,Wfac,pltOptConv,pltOptReb,span2Plt,...
...
dblowRight=zeros(1,3);
for i=1:nspans
    i1=(i-1)*3+1;
    i2=i*3;
    Aos3beam(1,1)=Ao3(i1,1);
    Aos3beam(2,1)=Ao3(i1+1,1);
    if nspans>1 
        if i< nspans
            Aos3beam(3,1)=max(Ao3(i2+1,1)+Ao3(i2,1));
        else
            Aos3beam(3,1)=Ao3(i2,1); 
        end
    else
        Aos3beam(3,1)=Ao3(i2,1);
    end
    ...
    bestnbcut3sec,nblowRight,dblowRight,CFA]=SAPSOBeamsRebar3DSec...
    ...
    nblowRight,Aos3beam,Wfac,pltOptConv,i,nMaxIter,numberOfParticles);
...
if pltOptReb==1
    for i=1:nspanplt
        ...
        %% Plot
        plotBeamBar3sec(b(i),h(i),DistrRebarLeftSec,ListRebarDiamLeftSec,...
