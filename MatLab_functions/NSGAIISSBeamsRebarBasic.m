function [extrOptPFDb,extrOptPFArea,extrOptPFCS,PF_CFA,PF_VOL,newPop,ff2,genCFA,genVOL,...
    ...
    CFA_PF_REF,Wfac,pccb,MIGDconv,pop_size,gen_max)
...
V=8; % number of optimization variables
M=2+15; % number of optimization functions + most important features of 
        % each individual
...
for i =1:pop_size % can be set to parfor
    ...
    dbTopMid(i,:),nblowRight(i,:),dblowRight(i,:),bestCFA(i),cc(i)]=CutRedistrOptimRecBeam1DSec(load_conditions,fcu,...
    Es,h,b,span,dbc,hagg,brec,hrec,pmin,pmax,sepMin,cutLoc,...
    Wfac,nblm,rebarAvailable);

    dbc2(i,:)=[dbc];
    db2c(i,:)=[dblowLeft(i,:),dbTopMid(i,:),dblowRight(i,:)];
    nb2c(i,:)=[nblowLeft(i,:),nbTopMid(i,:),nblowRight(i,:)];
...
optimal_features_ff=[bestCost NbCombo9 dbc2 bestEff];
...
for gen_count=1:gen_max
    ...
    for ii = 1:nchildren % can be set to parfor
        ...
        nb1l=1+fix(child_offspring(ii,3));
        nb2l=1+fix(child_offspring(ii,4));
        nb3l=1+fix(child_offspring(ii,5));

        nb1m=1+fix(child_offspring(ii,6));
        nb2m=1+fix(child_offspring(ii,7));
        nb3m=1+fix(child_offspring(ii,8));
        ...
        dbTopMid(ii,:),nblowRight(ii,:),dblowRight(ii,:),bestCFA(ii),cc(ii)]=CutRedistrOptimRecBeam1DSec(load_conditions,fcu,...
        Es,h,b,span,dbc,hagg,brec,hrec,pmin,pmax,sepMin,cutLoc,...
        Wfac,nblm,rebarAvailable);
    ...
    optimal_features_fff=[bestCost NbCombo9 dbc2 bestEff];
...
cscore=newPop(:,10);
bestnb9=newPop(:,12:20);
bestcombdi2=newPop(:,21:22);
for i=1:pop_size
