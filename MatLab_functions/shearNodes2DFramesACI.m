function [VcrxNode,VuxNode,VxSF,hnodeMin]=shearNodes2DFramesACI(ni,nf,...
 elembeams,elemcols,fcNodes,fy,MrBeamsCollec,steelAreaBeams3sec,...
 dimensions,nnodes,coordBaseCols,L)
%-------------------------------------------------------------------------
% SYNTAX : 
% [VcrxNode,VuxNode,VxSF,hnodeMin]=shearNodes2DFramesACI(ni,nf,elembeams,...
...
%     nodes, according to the ACI-352RS specifications, in units lb,in.
% 
...
for node=1:nnodes
    ...
    if nbeamNode==1 && ncolNode==2
        ...
        summhc=0;
        for i=1:nbeamNode
            summhc=summhc+m*hc*0.5; % assuming the dimensions of the 
                                    % larger column are greater than all
                                    % the beam width dimensions
        end
        be1=(bv+bc)*0.5;
        be2=bv+summhc;
        be3=bc;
        ...
        Vcrx=0.083*0.85*gamma*sqrt(fcNodes)*be*hc;
        VcrxNode(node,1)=Vcrx;
        VuxNode(node,1)=Vux;
        VxSF(node,1)=Vux/Vcrx;
        
        hnodeMin(node,1)=Vux/(0.083*0.85*gamma*sqrt(fcNodes)*be);
