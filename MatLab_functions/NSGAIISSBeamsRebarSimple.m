function [extrOptPFArea,extrOptPFCS,extrOptPFVol,extrOptPFNb,extrOptPFDb,...
    PF_CFA,PF_VOL,newPop,feasibleSol,genCFA,genVOL,IGDt,IGDv]=NSGAIISSBeamsRebarSimple(b,h,...
    span,brec,hrec,hagg,pmin,pmax,rebarAvailable,fcu,load_conditions,fy,wac,...
    cutLoc,dbcc,nbcc,dblow,nblow,W_PF_REF,CFA_PF_REF,Wfac,pccb,MIGDconv,...
    pop_size,gen_max)
...
V=12; % number of optimization variables
M=2+22; % number of optimization functions + most important features of 
        % each individual
...
for i =1:pop_size % can be set to parfor
    ...
    dbTopMid(i,:),nblowRight(i,:),dblowRight(i,:),bestCFA(i),cc(i)]=PlainCutRedistrOptimRecSSBeamSimple(load_conditions,fcu,...
    ...
    puBeam=unitCostCardBeamsRec(pccb(1),pccb(2),pccb(3),...
                             pccb(4)*bestCFA(i),pccb(5),pccb(6),pccb(7));

    bestCost(i,1)=rebarVol(i)*wac*puBeam;
...
for i=1:pop_size
    ff2=[ff2;
        [ff(i,:), bestCost(i,:), NbCombo9(i,:), dbc9(i,:), cc(i)]];
end

error_norm=normalisation(err); % Normalisation of the constraint violation

optimal_features_ff=[bestCost NbCombo9 dbc9 bestEff];
...
for gen_count=1:gen_max
    ...
    for ii = 1:nchildren % can be set to parfor
        ...
        nb1l=1+fix(child_offspring(ii,7));
        nb2l=1+fix(child_offspring(ii,8));
        nb3l=1+fix(child_offspring(ii,9));

        nb1m=1+fix(child_offspring(ii,10));
        nb2m=1+fix(child_offspring(ii,11));
        nb3m=1+fix(child_offspring(ii,12));
        ...
        DistrRebarRight{ii},dbcRight(ii,:),nbcut3sec(:,:,ii),nblowLeft(ii,:),dblowLeft(ii,:),nbTopMid(ii,:),...
        dbTopMid(ii,:),nblowRight(ii,:),dblowRight(ii,:),bestCFA(ii),cc(ii)]=PlainCutRedistrOptimRecSSBeamSimple(load_conditions,fcu,...
        Es,h,b,span,dbc,hagg,brec,hrec,pmin,pmax,sepMin,rebarAvailable,cutLoc,...
        Wfac,nblm);

        dbc9(ii,:)=[dbc,dbcRight(ii,:)];
        ...
        bestCost(ii,1)=rebarVol(ii)*wac*puBeam;
    ...
    optimal_features_fff=[bestCost NbCombo9 dbc9 bestEff];
...
cscore=newPop(:,14);
bestVols=newPop(:,13);
bestnb9=newPop(:,16:24);
bestcombdi9=newPop(:,25:33);
for i=1:pop_size
...
minOpt=bestVols(1,:);
min2Opt=bestVols(min2index,:);
midOpt=bestVols(midindex,:);
mid2Opt=bestVols(mid2index,:);
maxOpt=bestVols(pop_size,:);

extrOptPFVol=[minOpt,min2Opt,midOpt,mid2Opt,maxOpt];


minOpt=bestnb9(1,:);
min2Opt=bestnb9(min2index,:);
midOpt=bestnb9(midindex,:);
mid2Opt=bestnb9(mid2index,:);
maxOpt=bestnb9(pop_size,:);

extrOptPFNb=[minOpt,min2Opt,midOpt,mid2Opt,maxOpt];

minOpt=bestcombdi9(1,:);
min2Opt=bestcombdi9(min2index,:);
midOpt=bestcombdi9(midindex,:);
mid2Opt=bestcombdi9(mid2index,:);
maxOpt=bestcombdi9(pop_size,:);

extrOptPFDb=[minOpt,min2Opt,midOpt,mid2Opt,maxOpt];
