function [bestPerformance,bestLenRebar,bestsepRebar,bestPosition,bestdbc3,...
    ...
    bestCS]=SAPSOBeamsRebar1DSec(b,h,span,brec,hrec,hagg,pmin,pmax,rebarAvailable,...
    fcu,load_conditions,fy,wac,cutLoc,dbcc,nbcc,dblow,nblow,Wfac,Aos3,...
    graphConvPlot,ispan)
...
for j=1:nMaxIter
    ...
    for i=1:numberOfParticles
        ...
        nblo=ceil(Aos3(1)/abc(1)); % minimum required number of bars for left cross-section
        
        [nb1l,nb2l,nb3l,isfeasible]=nb1DSec1(nbmaxl,Aos3(1),abc(1));
        
        [nb1m,nb2m,nb3m,isfeasible]=nb1DSec1(nbmaxm,Aos3(2),abc(2));
        ...
        if isfeasible
            ...
            dbTopMid(i,:),nblowRight(i,:),dblowRight(i,:),CS(i),const(i)]=SACutRedistrOptimRecBeam1DSec(load_conditions,fcu,...
