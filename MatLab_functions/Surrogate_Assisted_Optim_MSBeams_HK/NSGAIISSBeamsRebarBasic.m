function [extrOptPFDb,extrOptPFArea,extrOptPFCS,PF_CFA,PF_VOL,newPop,ff2,genCFA,genVOL,...
    IGDt,IGDv]=NSGAIISSBeamsRebarBasic(b,h,span,brec,hrec,hagg,pmin,pmax,rebarAvailable,...
    fcu,load_conditions,fy,wac,cutLoc,dbcc,nbcc,dblow,nblow,W_PF_REF,...
    CFA_PF_REF,Wfac,Aos3,pccb,MIGDconv,pop_size,gen_max,pltPF)
...
M=2+24; % number of optimization functions + most important features of 
        % each individual
...
for i =1:pop_size % can be set to parfor
    ...
    nbmaxl=fix((b-2*brec-2*dvs+sepMin1l)/(dbc(1)+sepMin1l));
    nbmaxm=fix((b-2*brec-2*dvs+sepMin1m)/(dbc(2)+sepMin1m));
    ...
    dbTopMid(i,:),nblowRight(i,:),dblowRight(i,:),bestCS(i),cc(i)]=CutRedistrOptimRecBeam1DSec(load_conditions,fcu,...
    Es,h,b,span,dbc,hagg,brec,hrec,pmin,pmax,sepMin,cutLoc,...
    Wfac,nblm,Aos3);
    
    dbc2(i,:)=[dbc];
    db2c(i,:)=[dblowLeft(i,:),dbTopMid(i,:),dblowRight(i,:)];
    ...
    puBeam=unitCostCardBeamsRec(pccb(1),pccb(2),pccb(3),...
                             pccb(4)*bestCS(i),pccb(5),pccb(6),pccb(7));
...
for i=1:pop_size
    ff2=[ff2;
        [ff(i,:), bestCost(i,:), NbCombo9(i,:), dbc2(i,:), cc(i)]];
end

error_norm=normalisation(err); % Normalisation of the constraint violation

optimal_features_ff=[bestCost NbCombo9 dbc2 nb2c9 bestEff];
...
for gen_count=1:gen_max
    ...
    for ii = 1:nchildren % can be set to parfor
        %% Design optimization
        % Optimization design
        i1=1+fix(child_offspring(ii,1));
        i2=1+fix(child_offspring(ii,2));
        if i1>ndiam
            i1=ndiam;
        elseif i1<=0
            i1=1;
        ...
        db1l=rebarAvailable(i1,2);
        db1m=rebarAvailable(i2,2);
        ...
        dbTopMid(ii,:),nblowRight(ii,:),dblowRight(ii,:),bestCS(ii),cc(ii)]=CutRedistrOptimRecBeam1DSec(load_conditions,fcu,...
        Es,h,b,span,dbc,hagg,brec,hrec,pmin,pmax,sepMin,cutLoc,...
        Wfac,nblm,Aos3);

        dbc2(ii,:)=[dbc];
        
        db2c(ii,:)=[dblowLeft(ii,:),dbTopMid(ii,:),dblowRight(ii,:)];
        ...
        puBeam=unitCostCardBeamsRec(pccb(1),pccb(2),pccb(3),...
                             pccb(4)*bestCS(ii),pccb(5),pccb(6),pccb(7));
    ...
    for i=1:pop_size
        ff2=[ff2;
            [fff(i,:),bestCost(i,:),NbCombo9(i,:), dbc2(i,:), cc(i)]];
    end

    optimal_features_fff=[bestCost NbCombo9 dbc2 nb2c9 bestEff];
...
cscore=newPop(:,4);
bestnb9=newPop(:,6:14);
bestcombdi2=newPop(:,15:16);
for i=1:pop_size
    % Principal optimization faces
    bestcombAbi2=bestcombdi2(i,:).^2*pi/4;

    bestAbTL1=bestnb9(i,1).*bestcombAbi2(1);
    bestAbTL2=bestnb9(i,2).*bestcombAbi2(1);
    bestAbTL3=bestnb9(i,3).*bestcombAbi2(1);

    bestAbBM1=bestnb9(i,4).*bestcombAbi2(2);
    bestAbBM2=bestnb9(i,5).*bestcombAbi2(2);
    bestAbBM3=bestnb9(i,6).*bestcombAbi2(2);

    bestAbTR1=bestnb9(i,7).*bestcombAbi2(1);
    bestAbTR2=bestnb9(i,8).*bestcombAbi2(1);
    bestAbTR3=bestnb9(i,9).*bestcombAbi2(1);
...
minOpt=bestcombdi2(1,:);
min2Opt=bestcombdi2(min2index,:);
midOpt=bestcombdi2(midindex,:);
mid2Opt=bestcombdi2(mid2index,:);
maxOpt=bestcombdi2(pop_size,:);
