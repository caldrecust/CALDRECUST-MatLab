function [b,h,cost_elem_col,Ac_sec_elem,Ef_sec_col,Mr_col,t_value_x,...
        t_value_y,cxp,bestLoad]=isrColsInterSurf(pu_cols,height,b,h,rec,...
        fy,fc,load_conditions,wac,ductility,optimaConvPlot,plotISRResults)

%------------------------------------------------------------------------
% Syntax:
% [b,h,cost_elem_col,Ac_sec_elem,Ef_sec_col,Mr_col,t_value_x,...
%  t_value_y,cxy,bestLoad]=isrColsInterSurf(pu_cols,height,b,h,rec,fy,fc,...
...
% 
% Note: The structural efficiency is computed by rotating the cross-section
% according to each load combination given, so that an interaction diagram
% is computed for each of them with respect to their axis of action.
%
...
while Ac_sec_elem==0
    ...
    [Eftk1,tableEff,iloadmax,maxLoadCondition,maxgamma,diagramInterac,...
    rotdispositionRebar,rotSection,cxp,CP]=EffISRColsRot(b,h,tk1,...
    load_conditions,rec,fy,npdiag,fdpc,Es,betac);
    
    bestdiagramInterac=diagramInterac;
    bestRotSec=rotSection;
    bestLoad=maxLoadCondition;
    
    bestEfTable=tableEff(iloadmax,:);
    dt=0.00001;
    tk1dt=tk1+dt;
    
    cxk1=cxp;
    
    [Eftk1dt,tableEff,iloadmax,maxLoadCondition,maxgamma,diagramInterac,...
    rotdispositionRebar,rotSection,cxp,CP]=EffISRColsRot(b,h,tk1dt,...
    load_conditions,rec,fy,npdiag,fdpc,Es,betac);
    ...
    while (Eftk1>limEffSup || Eftk1<limEffInf)
        ...
        [Eftk2,tableEff,iloadmax,maxLoadCondition,maxgamma,diagramInterac,...
        rotdispositionRebar,rotSection,cxp,CP]=EffISRColsRot(b,h,tk2,...
        load_conditions,rec,fy,npdiag,fdpc,Es,betac);
        ...
        tdtk2=tk2+dt;
        [Eftdtk2,tableEff,iloadmax,maxLoadCondition,maxgamma,diagramInterac,...
        rotdispositionRebar,rotSection,cxp,CP]=EffISRColsRot(b,h,tdtk2,...
        load_conditions,rec,fy,npdiag,fdpc,Es,betac);
